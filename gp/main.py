...
filter_model_path = os.path.join(BASE_DIR, "blood_detector.h5")          # HDF5 model
cancer_model_path = os.path.join(BASE_DIR, "blood_cancer_model.keras")  # Keras 3 model

# === Load Models ===
try:
    filter_model = load_model(filter_model_path, compile=False)
    cancer_model = load_model(cancer_model_path, compile=False)
...
def predict():
    ...
    try:
        ...
        # --- Step 1: Filter model ---
        img_filter = image.load_img(io.BytesIO(file_bytes), target_size=(150, 150))
        img_array_filter = image.img_to_array(img_filter)
        img_array_filter = np.expand_dims(img_array_filter, axis=0) / 255.0

        filter_pred = filter_model.predict(img_array_filter)
        is_blood_image = filter_pred[0][0] < 0.5  # sigmoid output

        if not is_blood_image:
            return jsonify({
                "success": True,
                "is_blood_image": False,
                "prediction": "This is not a blood image. Please upload a valid blood cell image."
            }), 200

        # --- Step 2: Cancer model ---
        img_cancer = image.load_img(io.BytesIO(file_bytes), target_size=(224, 224))
        img_array_cancer = image.img_to_array(img_cancer)
